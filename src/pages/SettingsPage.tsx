...
const SettingsPage: React.FC = () => {
  const { settings, updateGeneralSettings, updateAppearanceSettings, updateAdvancedSettings, resetSettings, isInitialized } = useSettings();
  ...
  useEffect(() => {
    if (isInitialized) {
      ...
      readingForm.setFieldsValue(settings.appearance.reading);
    }
  ...
  const handleReadingSubmit = (values: any) => {
    updateAppearanceSettings({ reading: values });
            ...
            <TabPane tab="阅读" key="reading">
              {activeTab === 'reading' && (
                <Form
                  form={readingForm}
                  layout="vertical"
                  initialValues={settings.appearance.reading}
